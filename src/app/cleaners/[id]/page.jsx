...
async function getLocationData(slug) {
  ...
  const all = await Cleaner.find({}).select([
    'realName',
    'companyName',
    'rates',
    'services',
  ...
  return matched.map((cleaner) => ({
    ...
    servicesDetailed: Array.isArray(cleaner.servicesDetailed) ? cleaner.servicesDetailed.filter((s) => s?.active !== false) : [],
...
function buildFaqs(locationName, cleaners = []) {
  ...
  const allServiceNames = Array.from(new Set(cleaners.flatMap((c) => [
    ...(c.services || []),
...
function getNearbyLinks(locationSlug, cleaners = []) {
  const allSlugs = Array.from(new Set(cleaners.map((c) => slugify(c?.address?.town || c?.address?.county || '')).filter(Boolean)));
  ...
  if (!cleaners.length) {
    return (
      <main className="max-w-4xl mx-auto px-6 py-12">
        <h1 className="text-3xl font-bold text-teal-900 mb-4">No cleaners found in {locationName}</h1>
        <p className="text-slate-700">This location page is only published when real cleaner supply exists.</p>
      </main>
    );
  }
  ...
  return (
    <main className="max-w-6xl mx-auto px-6 py-10">
      ...
      <header className="mb-8">
        <h1 className="text-4xl font-bold text-teal-900 mb-3">Find cleaners in {locationName}</h1>
        <p className="text-slate-700 max-w-3xl">
          Browse real cleaner supply in {locationName}. Compare services, pricing style, and weekly availability before sending a booking request.
        </p>
      </header>

      <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 mb-10">
        {cleaners.map((cleaner) => (
          <article key={cleaner._id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-start justify-between gap-3 mb-3">
              <div>
                <h2 className="text-lg font-semibold text-slate-900">{cleaner.companyName || cleaner.realName}</h2>
                <p className="text-sm text-slate-500">{cleaner.address?.town || cleaner.address?.county || cleaner.address?.postcode || 'Location not set'}</p>
              </div>
              {cleaner.isPremium ? <span className="text-xs rounded-full bg-amber-100 text-amber-800 px-2 py-1">Premium</span> : null}
            </div>
            {cleaner.rates ? <p className="text-sm text-slate-700 mb-2"><strong>Hourly rate:</strong> £{cleaner.rates}</p> : null}
            {cleaner.servicesDetailed?.length ? (
              <ul className="text-sm text-slate-700 space-y-1 mb-4">
                {cleaner.servicesDetailed.slice(0, 4).map((svc) => (
                  <li key={svc.key || svc.name}>{svc.name} · {svc.defaultDurationMins || 60} mins{svc.price != null ? ` · £${svc.price}` : ''}</li>
                ))}
              </ul>
            ) : null}
            <Link href={`/cleaners/${cleaner._id}`} className="inline-flex rounded-xl bg-teal-600 text-white px-4 py-2 font-medium">
              View profile
            </Link>
          </article>
        ))}
      ...
      <section className="mb-10">
        <h2 className="text-2xl font-bold text-teal-900 mb-4">Frequently Asked Questions About Cleaners in {locationName}</h2>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-3">
            {nearbyLinks.map((slug) => (
              <Link key={slug} href={`/cleaners/${slug}`} className="rounded-full border border-slate-300 bg-white px-4 py-2 text-slate-700">
