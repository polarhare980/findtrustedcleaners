...
export default function ReviewTokenPageClient({ token: routeToken = '' }) {
  ...
  const token = useMemo(() => {
    return (
      ...
      String(searchParams.get('review') || '').trim()
    );
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      setMsg(
        j?.message ||
          (j?.success ? 'Thanks for your review.' : 'Unable to submit review.')
      );
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-8">
      <div className="max-w-3xl mx-auto">
        <div className="rounded-[28px] border border-slate-200 bg-white p-6 md:p-8 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-slate-600">
                Only customers with a real booking link can leave feedback. That helps keep reviews trustworthy.
              </p>
            </div>
          </div>
          ...
          {booking?.cleanerName ? (
            <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <div className="mt-1 text-sm text-slate-600">
                {[booking.serviceName, whenText, hourText].filter(Boolean).join(' • ')}
              </div>
            </div>
          ) : null}
          ...
          {!msg && blocked && eligibility?.message ? (
                ...
                <label className="flex items-start gap-3 text-sm text-slate-700">
                  ...
                  <span>
                    <span className="font-medium text-slate-900">
                      I would book this cleaner again
                    </span>
                    <span className="block text-slate-500">
                      This helps future customers understand trust and reliability.
                    </span>
                  </span>
                </label>
              </div>
