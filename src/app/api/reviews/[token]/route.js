...
function formatEligibilityMessage(reason) {
  switch (reason) {
    case 'too_early': return 'Reviews open shortly after the appointment time.';
    case 'review_window_closed': return 'This review link has expired.';
    case 'already_reviewed': return 'A review has already been submitted for this booking.';
    case 'booking_not_completed_enough': return 'This booking is not ready for review yet.';
    case 'missing_appointment_time': return 'We could not verify the appointment time for this booking.';
    default: return 'This review link is not valid.';
  }
}

async function loadPurchaseByToken(token) {
  const purchase = await Purchase.findOne({ reviewToken: token }).lean();
  if (!purchase) return null;
  const [cleaner, client, existingReview] = await Promise.all([
    Cleaner.findById(purchase.cleanerId, 'companyName realName').lean(),
    purchase.clientId ? Client.findById(purchase.clientId, 'fullName name email').lean() : null,
...
export async function GET(_req, { params }) {
  ...
  if (!token) return json({ success: false, message: 'Missing review token.' }, 400);

  await dbConnect();
  const loaded = await loadPurchaseByToken(token);
  if (!loaded) return json({ success: false, message: 'Review link not found.' }, 404);
  ...
  if (existingReview || purchase.reviewSubmittedAt) {
    return json({
      ...
      eligibility: { allowed: false, reason: 'already_reviewed', message: formatEligibilityMessage('already_reviewed') },
      booking: {
  ...
  return json({
    ...
    eligibility: { ...eligibility, message: formatEligibilityMessage(eligibility.reason) },
    booking: {
...
export async function POST(req, { params }) {
  ...
  if (existingReview || purchase.reviewSubmittedAt) {
    return json({ success: false, message: formatEligibilityMessage('already_reviewed') }, 409);
  }

  const eligibility = getReviewEligibility(purchase);
  if (!eligibility.allowed) {
    return json({ success: false, message: formatEligibilityMessage(eligibility.reason) }, 400);
  }
  ...
  const summary = agg[0] || {};
  await Cleaner.findByIdAndUpdate(purchase.cleanerId, { rating: summary.avg || 0, ratingCount: summary.count || 0 });
  await Purchase.updateOne({ _id: purchase._id }, { $set: { reviewSubmittedAt: new Date() } });
